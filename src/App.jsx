  ...
  try {
    ...
    if (raw) {
      ...
      return {
        ...
        zoneFindings: parsed.zoneFindings || {},
        player: {
...
export default function App() {
  ...
  function handleScoutZone(zoneId) {
    const zone = WORLD_ZONES.find((z) => z.id === zoneId);
    if (!zone?.scoutReward) return null;

    let rewardPayload = null;

    setState((prev) => {
      if (prev.zoneFindings?.[zoneId]) return prev;

      rewardPayload = { ...zone.scoutReward };
      const currentInventory = prev.player.inventory || [];
      const nextInventory = rewardPayload.item
        ? [...currentInventory, rewardPayload.item]
        : currentInventory;
      ...
      return {
        ...
        player: {
          ...prev.player,
          points: (prev.player.points || 0) + (rewardPayload.points || 0),
          fear: (prev.player.fear || 0) + (rewardPayload.fear || 0),
          investigation:
            (prev.player.investigation || 0) + (rewardPayload.investigation || 0),
  ...
  if (mode === "world") {
    return (
      <WorldScene
        ...
        zoneFindings={state.zoneFindings}
        onMove={(next) =>
          setState((prev) => ({
            ...prev,
        ...
        onScoutZone={(zoneId) => {
          const reward = handleScoutZone(zoneId);
          return reward;
        }}
  ...
  const currentZone = WORLD_ZONES.find((z) => z.sceneId === sceneId);
