...
const App = ({ bootstrap = {} }) => {
  ...
  const [state, setState] = useState(() => loadState(startSignature, bootstrap?.startParam));
  ...
  useEffect(() => {
    const refId = bootstrap?.startParam || new URLSearchParams(window.location.search).get('start');
    if (refId) {
      setState((prev) => upsertReferral(prev, refId));
    }
  }, [bootstrap?.startParam]);
